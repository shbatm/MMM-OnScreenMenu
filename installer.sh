...
if [ ! -d "$MM_HOME/modules/MMM-Remote-Control" ] ; then
    echo "The MMM-Remote-Control Module was not found. It is strongly recommended that you install this module as a pre-requisite."
    if check_yes "Would you like to install it now?"; then
        echo "OK, Installing MMM-Remote-Control. This script will resume once installed..."
        echo ""
        echo ""
...
if check_no "Do you want to view instructions on how to configure the module?"; then
    ...
    echo -e "        /* Valid positions: 'top_right', 'top_left', 'bottom_right', 'bottom_left' */"
    echo -e "        config: {"
    echo -e "            touchMode: true,"
    echo -e "            enableKeyboard: true,"
    echo -e "            // ... see more options in the README file"
    echo -e "        }"
    echo -e "    },"
